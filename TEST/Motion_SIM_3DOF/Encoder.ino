...
void Encoder(int PORT) { 
  ...
  }else if(digitalRead(Encoder_A[PORT]) == LOW) Encoder_A_Temp[PORT] = Encoder_Data[PORT]; // Pulse LOW
  ...
  }else if(digitalRead(Encoder_B[PORT]) == LOW) Encoder_B_Temp[PORT] = Encoder_Data[PORT]; // Pulse LOW
